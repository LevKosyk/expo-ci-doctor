...
import type { ReadinessScore } from '../utils/score-calculator.js';
import type { FailurePrediction } from '../analyzers/prediction.js';
import type { StabilityTrend } from '../analyzers/history.js';
 ...
 *
 * Includes summary, readiness score, issues table, failure prediction,
 * and stability trend. Designed to be pasted into GitHub Issues, PR comments,
 * or piped to a file for upload as a build artifact.
 */
export function renderMarkdown(opts: MarkdownReportOptions): string {
  const lines: string[] = [];
  const { results, readiness, prediction, trend, projectPath } = opts;
  ...
  // ─── Footer ──────────────────────────────────────────────────────
  lines.push('---');
  lines.push('*Generated by [expo-ci-doctor](https://expo-ci-doctor.com)*');
